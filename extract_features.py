...
class ExtractFeature:
    ...
    def extract(self, duration, days_back_to, date_from, date_to):
        ...
        for index, row in merged_data.iterrows():
            if pd.isnull(row['平均温度']):
                # 如果当前行没有层平均温度信息则跳过
                # print(row['层平均温度'])
                continue
            today['date'] = index
            today['grain_average'] = row['平均温度']
            ...
            td = pd.to_timedelta('1 days')  # 修改为1天
            ...
            for d in grain_dates:
                ...
                if d in self.data.index:
                    ...
                    before_data[str(day_back) + '_days_grain'] = round(float(self.data.loc[d]['平均温度']), 2)
                else:
...
if __name__ == '__main__':
    ...
    for layer in range(1, 5):
        for back_to in range(7,8):
            for barn in barns:
                ef = ExtractFeature()
                ef.merge_data_temp('data/rice_by_layer/rice_layer_' + str(layer) + '.csv', 'data/JiangSu.csv',
                                   barn=str(barn) + '仓')
                ef.extract(back_to + 5, back_to, pd.datetime(2014, 1, 1), pd.datetime(2017, 12, 30))
                output_dir = 'data/features/feature_rice_layer' + str(layer) + '_' + str(back_to) + 'days/'
