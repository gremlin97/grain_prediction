...
if __name__ == '__main__':
    ...
    # barns = [1, 8, 10, 12, 15, 16, 20, 26, 27, 28, 33]
    barns = [31]
    for day in range(15, 16):
        ...
        for x in range(0, 6):
            for y in range(0, 6):
                for z in range(0, 4):
                    images = np.array(IMGs)[:, x, y, z][:, np.newaxis]
                    features = np.array(FEAs)[:, :-1]
                    features = features[:, :7]
                    X = np.hstack((images, features))
                    Y = np.array(LABLEs)[:, z, y, x][:, np.newaxis]
                    # clf = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1,
                    #                                 max_depth=4, random_state=0, loss='lad')
                    # clf.fit(X, Y)
                    # joblib.dump(clf, './models/integer/day{}x{}y{}z{}.m'.format(day, x, y, z))
                    clf1 = joblib.load('./models/integer/day{}x{}y{}z{}.m'.format(day, x, y, z))
                    clf2 = joblib.load('./models/part/day{}x{}y{}z{}.m'.format(day, x, y, z))
                    scores_1.append(clf1.score(X, Y))
                    scores_2.append(clf2.score(X, Y))
        from matplotlib import pyplot as plt

        plt.figure()
        plt.plot(range(len(scores_1)), scores_1, c='r', label='interger')
        plt.plot(range(len(scores_2)), scores_2, c='b', ls='--', label='part')
        plt.legend()
        plt.show()
